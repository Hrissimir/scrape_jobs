...
def get_scrape_params(config: ConfigParser) -> ScrapeParams:
    what = config[SeekComAu.WHAT]
    where = config[SeekComAu.WHERE]
    days = config.getint(SeekComAu.KEY, SeekComAu.DAYS)
    tz = config[SeekComAu.TIMEZONE]
...
def get_upload_params(config: ConfigParser) -> UploadParams:
    name = config[Default.UPLOAD_SPREADSHEET_NAME]
    file = config[Default.UPLOAD_SPREADSHEET_JSON]
